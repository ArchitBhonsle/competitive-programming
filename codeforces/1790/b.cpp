...
void solve(int testcase) {
  int n_dice, sum, reduced_sum;
  cin >> n_dice >> sum >> reduced_sum;

  vector<int> seq = {sum - reduced_sum};
  int min_dice_val = reduced_sum / (n_dice - 1),
      remaining_val = reduced_sum % (n_dice - 1);

  for (int i = 0; i < n_dice - 1; ++i)
    seq.push_back(min_dice_val);
  for (int i = 1; i <= remaining_val; ++i)
    ++seq[i];
